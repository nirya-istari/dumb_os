...
impl SerialPort {
    ...
    /// Receives a byte on the serial port. Blocks waiting for input.
    pub fn receive(&mut self) -> u8 {
        unsafe {
            wait_for!(self.data_avaliable());
            self.data.read()
        }
    }
    
    /// Check if there is data avaliable for reading.
    pub fn data_avaliable(&mut self) -> bool {
